...
export function formatWorkingHours(minutes) {
  ...
  return `${padHrs}:${padMins} Hours`;
}

/**
 * Calculates exact working duration in minutes between Mark In and Mark Out datetimes.
 * Properly handles same-day, cross-midnight, and multi-day sessions.
 * Never resets at 00:00 midnight.
 * Truncates to minute precision so duration strictly matches the displayed Mark In and Mark Out HH:mm.
 */
export function calculateWorkingMinutes(markIn, markOut) {
  if (!markIn || !markOut) return 0;
  const inDate = markIn instanceof Date ? markIn : (parseKolkataDateTime(markIn) || new Date(markIn));
  const outDate = markOut instanceof Date ? markOut : (parseKolkataDateTime(markOut) || new Date(markOut));
  if (!inDate || !outDate || isNaN(inDate.getTime()) || isNaN(outDate.getTime())) return 0;
  if (outDate.getTime() <= inDate.getTime()) return 0;

  const inMinuteMs = Math.floor(inDate.getTime() / 60000) * 60000;
  const outMinuteMs = Math.floor(outDate.getTime() / 60000) * 60000;
  const diffMinutes = Math.round((outMinuteMs - inMinuteMs) / 60000);
  return Math.max(0, diffMinutes);
}

/**
 * Resolves complete authoritative Mark In Date object from an attendance record.
 */
export function getRecordMarkInDateTime(record) {
  if (!record) return null;
  const directCandidates = [
    record.markInAt,
    record.mark_in_datetime,
    record.inDateTime,
    record.loginTime,
    record.previousInDateTime,
  ];
  for (const c of directCandidates) {
    if (c) {
      const d = c instanceof Date ? c : (parseKolkataDateTime(c) || new Date(c));
      if (d && !isNaN(d.getTime())) return d;
    }
  }

  // Combined date and time fields
  const inTimeStr = record.inTime || record.mark_in_time || record.originalInTime;
  if (inTimeStr) {
    const dateStr = record.inDate || record.attendanceDate || record.date || record.originalInDate;
    if (dateStr) {
      const cleanDate = getAttendanceDateString(record) || String(dateStr).trim();
      const d = parseKolkataDateTime(`${cleanDate}T${String(inTimeStr).trim()}`);
      if (d && !isNaN(d.getTime())) return d;
    }
  }
  ...
  if (record.createdAt) {
    ...
    const rawType = String(record.attendanceType || '').toUpperCase();
    if (rawStatus !== 'ABSENT' && rawType !== 'ABSENT') {
...
/**
 * Resolves complete authoritative Mark Out Date object from an attendance record.
 */
export function getRecordMarkOutDateTime(record) {
  if (!record) return null;
  const directCandidates = [
    record.markOutAt,
    record.mark_out_datetime,
    record.outDateTime,
    record.newOutDateTime,
    record.logoutTime,
    record.lastDetectedOutAt,
  ];
  for (const c of directCandidates) {
    if (c) {
      const d = c instanceof Date ? c : (parseKolkataDateTime(c) || new Date(c));
  ...
  // Combined date and time fields
  const outTimeStr = record.outTime || record.mark_out_time || record.originalOutTime;
  if (outTimeStr) {
    const dateStr = record.outDate || record.inDate || record.attendanceDate || record.date;
    if (dateStr) {
      const cleanDate = String(record.outDate || getAttendanceDateString(record) || dateStr).trim();
      const d = parseKolkataDateTime(`${cleanDate}T${String(outTimeStr).trim()}`);
      if (d && !isNaN(d.getTime())) return d;
    }
  }
  return null;
...
export function getAttendanceDateString(record) {
  ...
  for (const c of candidates) {
    if (c) {
      ...
      const parsed = parseKolkataDateTime(str);
  ...
  const d = record.markInAt || record.mark_in_datetime || record.inDateTime || record.createdAt;
  if (d) {
    try {
      const dt = typeof d === 'string' ? parseKolkataDateTime(d) : d;
      if (dt && !isNaN(new Date(dt).getTime())) {
        return formatInTimeZone(new Date(dt), APP_TIMEZONE, 'yyyy-MM-dd');
      }
    } catch {
      return '';
    }
...
/**
 * Formats an attendance date into "Week Name, DD-MM-YYYY" (e.g. "Saturday, 19-09-2026")
 * strictly calculated in Asia/Kolkata timezone from actual database values.
 */
export function formatAttendanceDateWithWeek(dateInput) {
  if (!dateInput) return '-';
  try {
    let d;
    if (dateInput instanceof Date) {
      ...
      if (!str) return '-';
      if (/^\d{4}-\d{2}-\d{2}$/.test(str)) {
        d = parseKolkataDateTime(`${str}T00:00:00`);
      } else {
        d = parseKolkataDateTime(str) || new Date(str);
      }
    }
    if (!d || isNaN(d.getTime())) return String(dateInput);
    return formatInTimeZone(d, APP_TIMEZONE, 'EEEE, dd-MM-yyyy');
  } catch {
