...
import { getAttendanceDateString } from './timezone.js';
...
export function normalizeAttendance(doc) {
  ...
  const plantId = raw.plantId ? String(raw.plantId) : '';
  const plantName = raw.plantName || raw.inPlant || raw.outPlant || raw.street || 'Manufacturing Plant';
  ...
  if (!isExplicitAbsent) {
    if (raw.markInAt) {
      ...
      markInAt = new Date(`${raw.inDate}T${raw.inTime}:00`);
    }
  }
  ...
  if (!isExplicitAbsent) {
    if (raw.markOutAt) {
      ...
      markOutAt = new Date(`${raw.outDate}T${raw.outTime}:00`);
    }
  }
  ...
  const markInPlantName = raw.markInPlantName || raw.plantName || raw.inPlant || raw.street || (status === 'ABSENT' ? '-' : 'Manufacturing Plant');
  ...
  if (!markOutPlantName) {
    if (status === 'ABSENT') {
      ...
      markOutPlantName = markInPlantName;
    }
  }
