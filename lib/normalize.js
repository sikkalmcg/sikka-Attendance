...
export function normalizeAttendance(doc) {
  ...
  const isAutoRecord =
    status === 'AUTO_COMPLETED' ||
    Boolean(raw.autoMarkOut || raw.autoCheckout) ||
    String(raw.outType || '').toLowerCase().includes('auto') ||
    String(raw.markOutType || '').toLowerCase().includes('auto') ||
    raw.markOutPlantName === 'Auto-Out';

  if (isAutoRecord && markInAt) {
    // According to rule: saved Mark Out Date Time is Mark In Date Time + 8 hours
    markOutAt = new Date(markInAt.getTime() + 8 * 60 * 60 * 1000);
  }
  ...
  if (status === 'ABSENT' || status === 'ACTIVE') {
    ...
    // According to rule: every employee that auto Mark Out has working hours = 8 Hours (480 mins)
    workingMinutes = 8 * 60;
  ...
  return {
    ...
    autoOutTriggerTime: raw.autoOutTriggerTime ? new Date(raw.autoOutTriggerTime).toISOString() : null,
    approvalStatus,
    approved,
