      ...
      <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
        <div className="rounded-3xl border-none shadow-2xl overflow-hidden bg-white flex flex-col min-h-[calc(100vh-140px)]">
          ...
          <ScrollArea className="flex-1 bg-white">
            ...
            <TableBody>{data.length === 0 ? (<TableRow><TableCell colSpan={10} className="text-center py-20 text-muted-foreground italic">No active logs.</TableCell></TableRow>) : (data.map((rec: any, idx: number) => {
              const isHolidayDate = (holidays || []).some(hol => hol.date === rec.date) || isSunday(parseISO(rec.date));
              const isActuallyPresent = PRESENT_STATUSES.includes(rec.status);
              const displayStatus = (isActuallyPresent && isHolidayDate) ? "PRESENT ON HOLIDAY" : (rec.autoCheckout ? "AUTO_OUT" : rec.status);
              
              return (
                <TableRow key={idx} className="hover:bg-slate-50/50 transition-colors">
                  <TableCell className="px-8 py-6 font-black text-slate-700 uppercase text-sm"><div className="flex flex-col"><span>{rec.employeeName}</span><span className="text-[10px] font-mono font-black text-primary uppercase">{rec.employeeId}</span></div></TableCell>
                  ...
                  <TableCell className="text-xs font-mono font-bold"><div className="flex items-center gap-1.5"><Clock className="w-3 h-3 text-slate-400" />{formatDate(rec.date)} {rec.inTime || "--:--"}</div></TableCell>
                  <TableCell className="text-xs font-mono font-bold"><div className={cn("flex items-center gap-1.5", rec.autoCheckout ? "text-rose-500" : "text-slate-400")}><Clock className="w-3 h-3" />{rec.outTime ? `${formatDate(rec.outDate || rec.date)} ${rec.outTime}` : "In-Progress"}</div></TableCell>
                  <TableCell><Badge className={cn("text-[9px] font-black uppercase tracking-widest", isActuallyPresent ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-700")}>{displayStatus}</Badge></TableCell>
                  <TableCell className="pr-8"><div className="flex items-center gap-1.5 max-w-[300px]"><MapPin className="w-3.5 h-3.5 text-primary shrink-0" /><span className="text-[10px] font-bold text-slate-600 truncate">{rec.address || "N/A"}</span></div></TableCell>
                </TableRow>
              );
            }))}</TableBody></Table>
