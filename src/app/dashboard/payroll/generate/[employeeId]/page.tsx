...
export default function GenerateSalaryPage() {
  ...
  const { employees, attendanceRecords, payrollRecords, vouchers, addRecord, updateRecord } = useData();
  ...
  const currentSummary = useMemo(() => {
    ...
    if (queryEarningDays !== null) {
      ...
      return { attendance: eDays, absent: Math.max(0, 30 - eDays), holidayWork: 0, totalDays: 30 };
    }
    return null;
  ...
  const handlePostSalary = () => {
    ...
    try {
      ...
      toast({ title: "Salary Generated", description: "Payroll record updated." });
      router.push("/dashboard/payroll");
