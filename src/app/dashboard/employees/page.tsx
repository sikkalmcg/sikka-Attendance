...
import { Employee, SalaryStructure, StatusHistoryEntry } from "@/lib/types";
...
export default function EmployeesPage() {
  ...
  const handlePost = async () => {
    ...
    try {
      const finalData: any = { 
        ...
        isGovComplianceEnabled: formData.isGovComplianceEnabled ?? true,
  ...
  const handleStatusToggle = (emp: Employee) => {
    ...
    try {
      ...
      const modifier = verifiedUser?.fullName || "HR_ADMIN";

      const historyEntry: StatusHistoryEntry = {
        id: "st-" + Date.now(),
        oldStatus: emp.active ? "Active" : "Inactive",
        newStatus: newActive ? "Active" : "Inactive",
        inactiveDate: !newActive ? today : (emp.inactiveDate || ""),
        activeDate: newActive ? today : (emp.activeDate || ""),
        changedBy: modifier,
        changedOn: nowIso
      };
  ...
  const openEditModal = (emp: Employee) => {
    setEditEmployee(emp);
    setFormData({ ...INITIAL_FORM_DATA, ...emp });
    setIsModalOpen(true);
  };
  ...
  return (
    <TooltipProvider>
      <div className="space-y-6 pb-12">
                            ...
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <Button 
                                  ...
                                  onClick={() => { setHistoryEmp(emp); setIsHistoryOpen(true); }}
                                >
                            ...
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <Button 
                                  variant="ghost" 
                                  size="icon" 
                                  className={cn("h-8 w-8", emp.active ? "text-rose-500 hover:bg-rose-50" : "text-emerald-500 hover:bg-emerald-50")}
        ...
        {/* Status Transition Audit Log Dialog */}
        <Dialog open={isHistoryOpen} onOpenChange={(o) => !o && setIsHistoryOpen(false)}>
          <DialogContent className="sm:max-w-4xl p-0 overflow-hidden border-none shadow-2xl rounded-3xl">
            <DialogHeader className="p-8 bg-slate-900 text-white shrink-0">
               <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1">
                     <DialogTitle className="text-2xl font-black uppercase tracking-tight">{historyEmp?.name}</DialogTitle>
                     <p className="text-[11px] font-bold text-primary uppercase tracking-[0.2em] mt-1">Lifecycle Audit Trail & Access Logs</p>
                  </div>
               </div>
               <div className="grid grid-cols-2 sm:grid-cols-3 gap-6 pt-4 border-t border-white/10">
                  <div>
                     <Label className="text-[9px] font-black uppercase text-slate-400 tracking-widest">Employee ID</Label>
                     <p className="text-sm font-mono font-bold mt-0.5">{historyEmp?.employeeId}</p>
                  </div>
                  <div>
                     <Label className="text-[9px] font-black uppercase text-slate-400 tracking-widest">Current Status</Label>
                     <Badge className={cn("text-[10px] font-black uppercase block w-fit mt-0.5", historyEmp?.active ? "bg-emerald-500" : "bg-rose-500")}>
                        {historyEmp?.active ? "Active" : "Inactive"}
                     </Badge>
                  </div>
            ...
            <div className="p-8 bg-slate-50/50">
               <div className="flex items-center justify-between mb-4">
                  <h3 className="text-sm font-black uppercase tracking-[0.2em] text-slate-400 flex items-center gap-2">
                     <Clock className="w-4 h-4" /> Transition History
                  </h3>
               ...
               <Card className="border-slate-200 shadow-sm overflow-hidden rounded-2xl">
                  <Table>
                     <TableHeader className="bg-white">
                        <TableRow>
                           <TableHead className="font-black text-[10px] uppercase tracking-tighter py-5">Changed On</TableHead>
                           <TableHead className="font-black text-[10px] uppercase tracking-tighter">Transition</TableHead>
                           <TableHead className="font-black text-[10px] uppercase tracking-tighter">Inactive Date</TableHead>
                           <TableHead className="font-black text-[10px] uppercase tracking-tighter">Active Date</TableHead>
                           <TableHead className="font-black text-[10px] uppercase tracking-tighter text-right">Modified By</TableHead>
                        </TableRow>
                     </TableHeader>
                     <TableBody>
                        {(historyEmp?.statusHistory || []).length === 0 ? (
                           ...
                           [...(historyEmp?.statusHistory || [])].reverse().map((log) => (
                             <TableRow key={log.id} className="hover:bg-slate-50 transition-colors">
                                ...
                                <TableCell>
                                   <div className="flex items-center gap-2">
                                      <span className={cn("text-[9px] font-black uppercase px-2 py-0.5 rounded-md", log.oldStatus === 'Active' ? "bg-emerald-50 text-emerald-600" : "bg-rose-50 text-rose-600")}>{log.oldStatus}</span>
                                      <ChevronRight className="w-3 h-3 text-slate-300" />
                                      <span className={cn("text-[9px] font-black uppercase px-2 py-0.5 rounded-md", log.newStatus === 'Active' ? "bg-emerald-500 text-white" : "bg-rose-500 text-white")}>{log.newStatus}</span>
                                   </div>
                                </TableCell>
                                <TableCell className="text-xs font-bold text-rose-600">{log.inactiveDate ? formatDate(log.inactiveDate) : "---"}</TableCell>
                                <TableCell className="text-xs font-bold text-emerald-600">{log.activeDate ? formatDate(log.activeDate) : "---"}</TableCell>
                                <TableCell className="text-right text-[10px] font-black uppercase text-slate-500">{log.changedBy}</TableCell>
                             </TableRow>
                           ))
                        )}
                     </TableBody>
            ...
            <DialogFooter className="p-6 bg-white border-t flex items-center justify-between">
               ...
               <Button onClick={() => setIsHistoryOpen(false)} className="h-11 px-8 rounded-xl font-black bg-slate-900 hover:bg-primary transition-all">Close Audit Log</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
      </div>
