  ...
  return (
    <header className="sticky top-0 z-20 bg-white/90 backdrop-blur-md border-b border-slate-200/80 px-4 md:px-8 py-3.5 flex items-center justify-between shadow-xs">
      ...
      <div className="flex items-center gap-3 sm:gap-4">
        {/* Live Clock Badge */}
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-slate-100 border border-slate-200/70 text-slate-700 shadow-xs">
          <Clock className="w-3.5 h-3.5 text-blue-600 animate-pulse" />
          <div className="text-right">
            <div className="text-xs font-semibold tabular-nums tracking-tight">{timeStr || 'Loading...'}</div>
            <div className="text-[10px] text-slate-500 font-medium sm:hidden">{dateStr}</div>
          </div>
